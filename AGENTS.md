...
```text
00  MFO-WO-P2-2A-010 R5I Fail accepted as candidate harness preparation-details schema nonconformance; partial Stage／candidate-010 frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5J: candidate-011 exact one-line correction → bounded static audit → six compile／two parse; no Stage
 └─→ 20  MFO-WO-P2-20-001 package and administrative handoff frozen; no variant selected; no follow-on
       ↓
00  Review R5J compile／static closure only; no automatic Stage, PREACK, performance, integration, Gate 2, or Slice 2-B approval
 ↓
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Jだけである。
...
R5Jはcandidate-010からcandidate-011をexact 1回作り、共通`RoleContext.Execute`で`details = body();`直後に
`details["performance_slot_attempt_count"] = performanceAttemptCount;`をexact 1行だけ追加し、bounded static、fresh six
compile、two PowerShell parse-onlyまでを確認する。R5JではStagePreparer、Stage P、mode実行、PREPARED、PREACK、performance、
A／B／C real slot launch、game、user quiet windowを許可しない。Stage P再実行はR5J Pass後の別work orderでのみ扱う。
...
別work orderまで変更しない。R5J Passまたはpresentation proposalの行政受理だけではStage、PREACK、performance、Gate 2、Slice 2-Bは開かない。
