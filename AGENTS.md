...
```text
00  MFO-WO-P2-2A-010 R5B Fail accepted; qualification／compile／INIT passed, CONTRACT rejected candidate-008
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5C: candidate-009 LF normalization＋matcher-boundary correction → qualification／Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Cだけである。
...
`30`はR5Cでcandidate-008を凍結し、candidate-009へNativeのCRLF→LF byte normalizationとStagePreparerの
matcher-boundary correctionだけを適用し、static／compile／parse closureとfresh preparation flowを各1回実行する。
...
別work orderまで変更しない。R5C PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
