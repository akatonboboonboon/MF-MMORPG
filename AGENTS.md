...
```text
00  MFO-WO-P2-2A-010 R5D Fail accepted as external Git hunk-oracle false negative; candidate-010 exact and frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5E: qualify c4 read-only adoption of candidate-010 → compile／Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Eだけである。
...
candidate／game／performance defectとは扱わない。R5Eはfresh qualified c4でfrozen candidate-010をread-only採用し、
candidateを再作成・再書込せず、未実施のcompile／parse／fresh Stage Pだけを各1回実行する。CONTRACT内部LCSの
...
別work orderまで変更しない。R5E PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
