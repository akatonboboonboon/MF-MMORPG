...
```text
00  MFO-WO-P2-2A-010 R5E Blocked accepted as external RoleContext fixture typing error; candidate-010 exact and frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5F: qualify c5 one-expression fixture correction → compile／Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Fだけである。
...
game、performance defectとは扱わない。R5Fはfrozen c4からfresh c5を作り、この2次元配列生成式だけを
PowerShell 5で一意な`CreateInstance(Type, Int32, Int32)`へ直す。candidate-010を再作成・再書込せず、
QUALIFY Pass後だけ未実施のcompile／parse／fresh Stage Pを各1回実行する。CONTRACT内部LCSの
...
別work orderまで変更しない。R5F PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
