...
```text
00  MFO-WO-P2-2A-010 R5H Blocked accepted as external diff-audit helper syntax defect before Git diff; c7／empty root frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5I: qualify external audit driver → fresh c8 flat R5H-lineage binding → diff／QUALIFY／conditional Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Iだけである。
...
invocation`として受理し、c7、candidate-010、production harness、game、performance defectとは扱わない。c7は
`ExecutionHead=1b520e6…`を固定するため新監督commit後に直接実行できない。R5Iはfresh external audit driverを実diff前に
exact 1回資格確認し、Pass後だけfrozen c7からfresh c8をexact 1回作る。c8は機械的rolloverとflatなR5H凍結lineage
bindingだけを許可し、qualified driverでdiffを1回、Pass後にQUALIFYを1回、さらにPass時だけ未実施のcompile／parse／
fresh Stage Pを1回実行する。CONTRACT内部LCSの`54 / 54` authorized、unauthorized `0`は引き続き必須である。
...
別work orderまで変更しない。R5I PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
