...
```text
00  MFO-WO-P2-2A-010 R5F Blocked accepted as external frozen-lineage manifest schema mismatch; candidate-010 exact and frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5G: qualify c6 exact manifest-schema dispatch → bounded fixture／compile／Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Gだけである。
...
この既知2 schemaへのexact field dispatchだけを修正する。candidate-010を再作成・再書込せず、QUALIFY Pass後だけ
未実施のcompile／parse／fresh Stage Pを各1回実行する。CONTRACT内部LCSの
...
別work orderまで変更しない。R5G PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
