...
```text
00  MFO-WO-P2-2A-010 R5 Blocked / external driver Unicode decoding accepted; candidate／compile／Stage untouched
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5A: qualify one ASCII-only driver → fresh tool build／Stage P／PREPARED closure
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Aだけである。
...
R5はBOMなしUTF-8 driverが日本語repository pathを誤読し、compile／Stage／six-modeを全て`0`のまま
`Blocked / external orchestration driver Unicode decoding before tool compile`で停止した。candidate／harness／game／
performanceのFailではない。`30`はR5AでASCII-only driverと実Unicode pathをformal前に一度だけ資格確認し、合格時だけ
candidate-008を編集せずfresh tool buildと既存preparation flowを1回実行し、Section 3のsource-diff audit、A／B／C
identity、全6 mode、manifest／receipt／preparation audit、ReadOnly sealとPREPARED返却だけを行う。
...
別work orderまで変更しない。R5A PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
