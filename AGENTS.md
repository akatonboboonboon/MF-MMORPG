...
```text
00  MFO-WO-P2-2A-010 R5J Blocked accepted as external clone-tree ReadOnly precondition mismatch; candidate-011 seed-identical and frozen
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5J-A: fresh candidate-012 exact one-line correction → bounded static audit → six compile／two parse; no Stage
 └─→ 20  MFO-WO-P2-20-001 package and administrative handoff frozen; no variant selected; no follow-on
       ↓
00  Review R5J-A compile／static closure only; no automatic Stage, PREACK, performance, integration, Gate 2, or Slice 2-B approval
 ↓
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5J-Aだけである。
...
candidate correction、production harness、game、performance defectとは扱わない。R5J-Aはcandidate-010／011を凍結したまま
fresh candidate-012をexact 1回cloneし、clone直後は属性をacceptance gateにせず`8 / 8` byte identityを確認してから既存
`Freeze-Tree`をexact 1回適用する。その後candidate-012 Nativeだけをclear／exact 1行write／`finally` restoreし、bounded static、
fresh six compile、two PowerShell parse-onlyまでを確認する。R5J-AではStagePreparer、Stage P、mode実行、PREPARED、PREACK、
performance、A／B／C real slot launch、game、user quiet windowを許可しない。Stage P再実行はR5J-A Pass後のR5Kでのみ扱う。
...
別work orderまで変更しない。R5J-A Passまたはpresentation proposalの行政受理だけではStage、PREACK、performance、Gate 2、Slice 2-Bは開かない。
