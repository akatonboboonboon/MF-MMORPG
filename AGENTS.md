...
```text
00  MFO-WO-P2-2A-010 R5C Blocked accepted; candidate-009 remains seed-identical, correction／compile／Stage all 0
 ↓
00  MFO-HOLD-P2-2A-001 remains active; Gate 2 Locked; Slice 2-B unauthorized
 ├─→ 30  Recovery Step R5D: qualify PS5-compatible c3 promotion → candidate-010／compile／Stage P
...
現在のQA実行例外は
[`MFO-WO-P2-2A-010`](docs/work-orders/phase2-slice2a-qualified-harness-performance-acceptance.md)のRecovery Step R5Dだけである。
...
Pass時だけFORMAL内でcandidate promotion、compile／parse、fresh preparation flowを各1回実行する。
...
別work orderまで変更しない。R5D PREPAREDまたはpresentation proposalの行政受理だけではPREACK、performance、Gate 2、Slice 2-Bは開かない。
