...
func _test_runtime_configuration_and_rejection() -> void:
	...
	_check(empty_rejected and unknown_rejected and _is_final_idle(runtime) and runtime.debug_state()["accepted_count"] == 0 and runtime.debug_state()["query_count"] == 0 and _requests.is_empty(), "runtime rejects empty and unknown actions")
...
func _test_quick_runtime_contract() -> void:
	...
	_check(request.has("geometry") and request["geometry"] is Dictionary and request["geometry"].is_read_only() and request["geometry"].has("hit_shape_id") and request["geometry"]["hit_shape_id"] == &"hit_shape.physical.prototype.safe_circle" and request["geometry"].has("reach") and request["geometry"]["reach"] == 150.0 and request["geometry"].has("query_radius") and request["geometry"]["query_radius"] == 88.0 and request["geometry"].has("minimum_aim_dot") and request["geometry"]["minimum_aim_dot"] == 0.25 and request.has("max_targets") and request["max_targets"] == 1, "quick request snapshot preserves geometry and target cap")
	_check(request.has("effects") and request["effects"] is Array and request["effects"].is_read_only() and request["effects"].size() == 2 and request["effects"][0] is Dictionary and request["effects"][0].is_read_only() and request["effects"][1] is Dictionary and request["effects"][1].is_read_only() and request["effects"][0].has("effect_id") and request["effects"][0]["effect_id"] == &"effect.physical.quick_cut.damage" and request["effects"][0]["magnitude"] == 10.0 and request["effects"][1].has("effect_id") and request["effects"][1]["effect_id"] == &"effect.physical.quick_cut.part_damage" and request["effects"][1]["magnitude"] == 6.0 and request["effects"][0]["channel"] == &"physical" and request["effects"][0]["duration"] == 0.0 and request["effects"][0]["target_rule"] == &"hit_target" and request["effects"][0]["stack_rule"] == &"instant" and request["effects"][0]["tags"] is Array and request["effects"][0]["tags"].is_empty() and request["effects"][1]["channel"] == &"physical" and request["effects"][1]["duration"] == 0.0 and request["effects"][1]["target_rule"] == &"hit_target" and request["effects"][1]["stack_rule"] == &"instant" and request["effects"][1]["tags"] is Array and request["effects"][1]["tags"].is_empty(), "quick request effects are readonly snapshots")
	...
	_check(_is_final_idle(runtime) and _requests.size() == 1 and runtime.debug_state()["query_count"] == 1 and not runtime.advance(0.1) and _requests.size() == 1, "quick ends idle with no pending result/effect/lease")
...
func _test_heavy_runtime_contract() -> void:
	...
	_check(_requests.size() == 1 and heavy_request.is_read_only() and heavy_request.has("action_id") and heavy_request["action_id"] == Phase2CombatFormDefinition.ACTION_HEAVY_CLEAVE and heavy_request.has("accepted_sequence") and heavy_request["accepted_sequence"] == 1 and heavy_request.has("locked_aim") and heavy_request["locked_aim"] is Vector2 and heavy_request["locked_aim"].is_equal_approx(Vector2.RIGHT), "heavy ignores changed aim after acceptance")
	_check(_requests.size() == 1 and heavy_request.has("forward_movement_intent_pixels") and heavy_request["forward_movement_intent_pixels"] == 48.0 and heavy_request.has("geometry") and heavy_request["geometry"] is Dictionary and heavy_request["geometry"].is_read_only() and heavy_request["geometry"].has("hit_shape_id") and heavy_request["geometry"]["hit_shape_id"] == &"hit_shape.physical.prototype.safe_circle" and heavy_request["geometry"].has("reach") and heavy_request["geometry"]["reach"] == 150.0 and heavy_request["geometry"].has("query_radius") and heavy_request["geometry"]["query_radius"] == 88.0 and heavy_request["geometry"].has("minimum_aim_dot") and heavy_request["geometry"]["minimum_aim_dot"] == 0.25 and heavy_request.has("max_targets") and heavy_request["max_targets"] == 1 and heavy_request.has("effects") and heavy_request["effects"] is Array and heavy_request["effects"].is_read_only() and heavy_request["effects"].size() == 2 and heavy_request["effects"][0] is Dictionary and heavy_request["effects"][1] is Dictionary and heavy_request["effects"][0].is_read_only() and heavy_request["effects"][1].is_read_only() and heavy_request["effects"][0]["magnitude"] == 14.0 and heavy_request["effects"][1]["magnitude"] == 18.0 and heavy_request["effects"][0]["channel"] == &"physical" and heavy_request["effects"][0]["duration"] == 0.0 and heavy_request["effects"][0]["target_rule"] == &"hit_target" and heavy_request["effects"][0]["stack_rule"] == &"instant" and heavy_request["effects"][0]["tags"] is Array and heavy_request["effects"][0]["tags"].is_empty() and heavy_request["effects"][1]["channel"] == &"physical" and heavy_request["effects"][1]["duration"] == 0.0 and heavy_request["effects"][1]["target_rule"] == &"hit_target" and heavy_request["effects"][1]["stack_rule"] == &"instant" and heavy_request["effects"][1]["tags"] is Array and heavy_request["effects"][1]["tags"].is_empty(), "heavy request carries 48 pixel active-only intent")
	...
	_check(_is_final_idle(runtime) and _requests.size() == 1 and runtime.debug_state()["query_count"] == 1 and not runtime.advance(0.1) and _requests.size() == 1, "heavy ends idle without queued action")
...
func _test_complete_callback_and_boundary_contract() -> void:
	...
	_check(rejected_runtime.debug_result().has("release_succeeded") and rejected_runtime.debug_result()["release_succeeded"] and rejected_runtime.debug_state()["active_query_count"] == 0 and rejected_runtime.debug_state()["emergency_active_count"] == 0 and rejected_runtime.debug_state()["emergency_use_count"] == 0 and not rejected_runtime.debug_state()["has_pending_lease"], "rejected callback releases lease immediately without emergency use")
	...
	_check(reset_runtime.reset() and _requests.is_empty() and _is_final_idle(reset_runtime) and reset_runtime.debug_state()["active_query_count"] == 0 and reset_runtime.debug_state()["emergency_active_count"] == 0 and reset_runtime.debug_state()["emergency_use_count"] == 0, "reset-before-active releases without callback")
	...
	_check(clear_runtime.clear() and _requests.is_empty() and _is_final_idle(clear_runtime) and clear_runtime.debug_state()["active_query_count"] == 0 and clear_runtime.debug_state()["emergency_active_count"] == 0 and clear_runtime.debug_state()["emergency_use_count"] == 0, "clear-before-active releases without callback")
