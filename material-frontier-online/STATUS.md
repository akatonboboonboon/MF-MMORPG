...
- Current phase: **Phase 2 / Slice 2-A Stage B Fail / bounded correction**
- Phase 2 implementation: **MFO-WO-P2-2A-002 active / P1 correction pending**
...
- Active work order: [`../docs/work-orders/phase2-slice2a-nonzero-direction-correction.md`](../docs/work-orders/phase2-slice2a-nonzero-direction-correction.md)
- Returned work order: [`../docs/work-orders/phase2-slice2a-basic-operation.md`](../docs/work-orders/phase2-slice2a-basic-operation.md)
- Formal Slice 2-A QA: [`../docs/test-reports/phase2-slice2a-validation.md`](../docs/test-reports/phase2-slice2a-validation.md)
...
`MFO-WO-P2-2A-001`のStage A候補は実装されたが、formal QAは`119 / 120 Pass`、exit `1`でFailとなった。
`MFO-P2-2A-QA-001`は、input deadzone後もnonzeroの小moveをneutral扱いしてaimへfallbackするP1受入
blockerで、runtime影響はLowである。

現在は`MFO-WO-P2-2A-002`に従い、`10`がauthority／actorの2つのnonzero判定だけを是正し、handoff後に
`30`がfresh再検証する。物理gamepad証跡はOD-013を維持したままGate Playabilityまで延期し、KBM結果で
Passへ置き換えない。正式な快斬・重断、損傷、HUD統合、素材、魔法、boss、stageは別work orderまで
着手しない。
