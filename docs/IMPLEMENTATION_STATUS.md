...
| Next authority | `MFO-WO-P2-2A-010` remains pre-PREPARED: R5 may use immutable candidate-008 for exactly one end-to-end fresh Stage P preparation attempt; inside it StagePreparer runs `INIT`→`CONTRACT`→`SEAL` exactly once each (`3` launches total) and all six modes run exactly once before ReadOnly seal／PREPARED return; no candidate edit, retry, PREACK, performance, or game |
...
| Current authority | Recovery Step R5: immutable candidate-008 only; exactly one end-to-end fresh external Stage P preparation attempt, with StagePreparer `INIT`／`CONTRACT`／`SEAL` each exactly once (`3` launches total), all six modes exactly once, source-diff and A／B／C identity audit, real slot count `0`, complete manifest／receipt／preparation audit, ReadOnly seal, and PREPARED evidence return |
