...
namespace MfoQaQualification
{
    ...
    {
        public const string WorkOrder = "MFO-WO-P2-2A-009";
    ...
    public sealed class EvidenceJournal
    {
        ...
        public Dictionary<string, object> ReadRecordByDigest(string recordSha256)
        {
            using (Mutex mutex = new Mutex(false, mutexName))
            {
                bool acquired = false;
                ...
                while (!acquired)
                {
                    ...
                    if (!acquired)
                    {
                        if (NativeApi.GetTickCount64() >= acquisitionDeadline) throw new HarnessException("Journal mutex native-tick acquisition timeout during record readback");
                        Thread.Sleep(2);
                    }
                }
                try
                {
                    ...
                    for (int i = 0; i < lines.Length; i++)
                    {
                        ...
                        if (String.Equals(Record.Text(entry, "record_sha256"), recordSha256, StringComparison.Ordinal)) return entry;
                    }
                    throw new HarnessException("Durable journal record digest was not found during readback");
                }
                finally { if (acquired) mutex.ReleaseMutex(); }
            }
        }
    ...
    public static class HarnessOps
    {
        ...
        public static void ValidateRolePaths(string role, string mode, string stage, string identity, string output, string journal, string result)
        {
            ...
            else if (mode == "QP_LIVE_EVIDENCE_CONTRACT_SELFTEST") runRoot = Path.Combine(stage, "preparation", "live-evidence-contract-selftest-qualified");
        ...
        {
            Dictionary<string, object> persisted = PersistObservationReadback(journal, journalKind, observationPath, observation);
            byte[] bytes = File.ReadAllBytes(Record.Text(persisted, "pending_path"));
            Dictionary<string, object> readback = Record.AsMap(Record.Parse(Encoding.UTF8.GetString(bytes)));
            bool complete = HasCompleteObservationFieldSet(readback, requiredFields, requiredSchema);
            if (!complete) throw new HarnessException("Pending observation readback lacked the complete required field set");
            persisted["field_completeness_success"] = true;
            return persisted;
        }

        public static Dictionary<string, object> PersistObservationReadback(EvidenceJournal journal, string journalKind, string observationPath, Dictionary<string, object> observation)
        {
            ...
            return Record.Map("pending_path", full, "pending_sha256", EvidenceIo.Sha256Bytes(bytes), "readback_success", true);
        }

        public static bool HasCompleteObservationFieldSet(Dictionary<string, object> observation, string[] requiredFields, string requiredSchema)
        {
            for (int i = 0; i < requiredFields.Length; i++) if (!observation.ContainsKey(requiredFields[i]) || observation[requiredFields[i]] == null) return false;
            return String.Equals(SafeText(observation, "schema"), requiredSchema, StringComparison.Ordinal) &&
                String.Equals(SafeText(observation, "evaluation"), "pending", StringComparison.Ordinal);
        }
        ...
        private static readonly string[] LiveActivationPendingRequiredFields = new string[]
        {
        ...
        private static readonly string[] LauncherLivePendingRequiredFields = new string[]
        {
            "schema", "evaluation", "work_order_protocol", "pending_observation_path", "cli_contract_capture", "invocation_capture",
        ...
        public static bool HasCompleteLiveActivationPendingFieldSet(Dictionary<string, object> pending)
        {
            return HasCompleteObservationFieldSet(pending, LiveActivationPendingRequiredFields, Contract.ActivationPendingSchema);
        }

        public static bool HasCompleteLauncherLivePendingFieldSet(Dictionary<string, object> pending)
        {
            return HasCompleteObservationFieldSet(pending, LauncherLivePendingRequiredFields, Contract.ActivationPendingSchema);
        }
        ...
        {
            return PersistCompleteObservation(journal, "live_activation_pending_observation", pendingPath, pending, LiveActivationPendingRequiredFields, Contract.ActivationPendingSchema);
        }
            ...
            bool validateBindings)
        {
            ...
            bool complete = HasCompleteLiveActivationPendingFieldSet(pending);
            Dictionary<string, object> cliCapture = CaptureMap(pending, "cli_contract_capture");
            Dictionary<string, object> cli = CaptureValueMapOrEmpty(cliCapture);
            ...
            bool mechanism = readback && complete && preackTickValid && CaptureSucceededNoThrow(activationCapture) && activationPathMatch && slotMatch && bindingMatch && (cliPathMatch || !validateBindings) && (runtimeMatch || !validateBindings);
        ...
        {
            return PersistCompleteObservation(journal, "launcher_live_pending_observation", pendingPath, pending, LauncherLivePendingRequiredFields, Contract.ActivationPendingSchema);
        }
            ...
            bool validateBindings)
        {
            ...
            bool complete = HasCompleteLauncherLivePendingFieldSet(pending);
            Dictionary<string, object> cliCapture = CaptureMap(pending, "cli_contract_capture");
            Dictionary<string, object> cli = CaptureValueMapOrEmpty(cliCapture);
            Dictionary<string, object> manifestFile = CaptureValueMapOrEmpty(CaptureMap(pending, "manifest_capture"));
            Dictionary<string, object> manifest = MapOrEmpty(manifestFile, "document");
            ...
            bool mechanism = readback && complete && cliProtocolValid && CaptureSucceededNoThrow(exactCapture) && slotMatch && (runtimeMatch || !validateBindings) && (cliPathMatch || !validateBindings) && bindingMatch && (tickMatch || !validateBindings);
        ...
        public static Dictionary<string, object> BuildLiveSample(
            int n,
            ulong target,
            ulong actual,
            List<Dictionary<string, object>> oneDrive,
            Dictionary<string, object> host,
            List<Dictionary<string, object>> forbidden,
            List<Dictionary<string, object>> qaInventory,
            List<Dictionary<string, object>> descendants)
        {
            return Record.Map(
                "schema", "mfo.qa.live.sample.v1",
                "n", n,
                "target_tick", target.ToString(CultureInfo.InvariantCulture),
                ...
                "onedrive_processes", oneDrive,
                "host", host,
                "forbidden_runtime_count", forbidden.Count,
                "forbidden_runtime_processes", forbidden,
                "owned_process_inventory", qaInventory,
                "runner_descendant_inventory", descendants,
                "owned_allowed_roles", new string[] { "runner", "launcher", "controller" },
                "performance_slot_launch_count", 0);
        }

        public static Dictionary<string, object> PersistAndValidateLiveSample(EvidenceJournal journal, Dictionary<string, object> sample)
        {
            string digest = journal.Append("live_sample", sample);
            Dictionary<string, object> record = journal.ReadRecordByDigest(digest);
            if (!String.Equals(SafeText(record, "kind"), "live_sample", StringComparison.Ordinal)) throw new HarnessException("Durable live sample journal kind mismatch");
            Dictionary<string, object> readback = Record.AsMap(record["payload"]);
            object slotValue;
            bool slotPresent = readback.TryGetValue("performance_slot_launch_count", out slotValue) && slotValue != null;
            bool slotNumericZero = slotPresent && slotValue is Int32 && ((Int32)slotValue) == 0;
            if (!slotNumericZero) throw new HarnessException("Durable live sample performance slot field was missing, nonnumeric, or nonzero");
            return Record.Map(
                "record_sha256", digest,
                "sequence", Record.Integer(record, "sequence"),
                "native_tick", Record.Text(record, "native_tick"),
                "readback_success", true,
                "hash_chain_valid", true,
                "payload_readback", readback,
                "performance_slot_launch_count", 0);
        }
        ...
        public static string RequireExactActivation(string activationPath, string stageId, string manifestSha, string receiptSha, string preparationAuditSha, string preackSha, string preackEvaluationSha, ulong preackTick)
        {
            ...
            byte[] expectedBytes = EvidenceIo.Utf8(expected);
            if (!EvidenceIo.ByteEqual(bytes, expectedBytes)) throw new HarnessException("Activation token is not an exact START_ACK echo");
    ...
    public static class SentinelExercise
    {
        public static Dictionary<string, object> Run(
            ...
            JobScope owner,
            string label,
            ProcessIdentity runner,
            ProcessIdentity[] expectedQaBeforeChild,
            ProcessIdentity[] expectedDescendantsBeforeChild)
        {
            if (runner == null || expectedQaBeforeChild == null || expectedDescendantsBeforeChild == null) throw new HarnessException("Sentinel ownership expectations were missing");
            ...
            using (EventWaitHandle release = new EventWaitHandle(false, EventResetMode.ManualReset, releaseName))
            {
                ...
                bool rawOk = false;
                Dictionary<string, object> stdoutEvidence = null;
                Dictionary<string, object> stderrEvidence = null;
                using (OwnedChild child = OwnedChild.Start(HarnessOps.Bin(context.Stage, "MfoQaSentinel.exe"), sentinelArguments, null, owner, sentinelOut, sentinelErr, context.Journal, label, deadline))
                {
                    ...
                    if (readyObserved)
                    {
                        try
                        {
                            ...
                            {
                                List<ProcessIdentity> expectedQa = new List<ProcessIdentity>(expectedQaBeforeChild);
                                expectedQa.Add(child.Identity);
                                try { HarnessOps.RequireExactQaInventory(qaInventory, expectedQa.ToArray()); }
                                catch (Exception failure) { preReleaseDefect = "Sentinel owned-process inventory mismatch: " + failure.Message; }
                            }
                            if (preReleaseDefect == null)
                            {
                                List<ProcessIdentity> expectedDescendants = new List<ProcessIdentity>(expectedDescendantsBeforeChild);
                                expectedDescendants.Add(child.Identity);
                                try { HarnessOps.RequireExactDescendantInventory(descendants, expectedDescendants.ToArray()); }
                    ...
                    try { exit = child.WaitUntil(origin, deadline, owner); }
                    catch (Exception failure)
                    {
                        waitFailure = failure;
                    ...
                    rawOk = rawReadError == null && stdout == Contract.SentinelReady + Environment.NewLine + Contract.SentinelStdout + Environment.NewLine && stderr == Contract.SentinelStderr + Environment.NewLine;
                    stdoutEvidence = child.StdoutEvidence();
                    stderrEvidence = child.StderrEvidence();
                }
                uint remainingJobProcesses = owner.ActiveProcesses;
                context.Journal.Append("sentinel_complete", Record.Map(
                    "label", label,
                    "identity", childIdentity.ToRecord(),
                    "exit_code", exit,
                    "stdout", stdoutEvidence,
                    "stderr", stderrEvidence,
                    "raw_tokens_exact", rawOk,
                    "raw_read_error", rawReadError,
                    "wait_failure", waitFailure == null ? null : waitFailure.GetType().FullName + ": " + waitFailure.Message,
                    "owned_child_disposed", true,
                    "remaining_job_processes", remainingJobProcesses,
                    "performance_slot_launch_count", 0));
                if (waitFailure != null) throw waitFailure;
                if (handshakeFailure != null) throw handshakeFailure;
                if (remainingJobProcesses != 0) throw new HarnessException("Sentinel job retained active processes after child disposal");
                ...
                return Record.Map("identity", childIdentity.ToRecord(), "exit_code", exit, "raw_tokens_exact", rawOk, "stdout", sentinelOut, "stderr", sentinelErr, "owned_child_disposed", true, "remaining_job_processes", remainingJobProcesses);
            }
        }
    }
    ...
    public static class RunnerRole
    {
        ...
        private static Dictionary<string, object> Run(RoleContext c)
        {
            ...
            if (c.Mode == "QP_DRYRUN")
            {
                ...
                c.Journal.Append("qp_dryrun_complete", Record.Map("allowed_windows_console_host_count_before", allowedConsoleHostsBefore, "allowed_windows_console_host_count_after", allowedConsoleHostsAfter, "unexpected_descendant_count", 0, "performance_slot_launch_count", 0, "abc_launch_count", 0, "final_owned_runtime_count", 0));
                return Record.Map("dry_run", "pass", "forbidden_runtime_before_count", before.Count, "forbidden_runtime_after_count", after.Count, "qa_owned_before", qaBefore, "qa_owned_after", qaAfter, "runner_descendants_before", descendantsBefore, "runner_descendants_after", descendantsAfter, "allowed_windows_console_host_count_before", allowedConsoleHostsBefore, "allowed_windows_console_host_count_after", allowedConsoleHostsAfter, "unexpected_descendant_count", 0, "performance_slot_launch_count", 0, "abc_launch_count", 0, "final_owned_runtime_count", 0, "identity_audit", HarnessOps.FileSetAudit(identity, c.Stage));
            }
            ...
            if (c.Mode == "QP_LIVE_EVIDENCE_CONTRACT_SELFTEST") return RunLiveEvidenceContractSelfTest(c, identity);
        ...
        private static Dictionary<string, object> RunLiveEvidenceContractSelfTest(RoleContext c, string identity)
        {
            Dictionary<string, object> manifest = EvidenceIo.ReadMap(identity);
            string stageId = Record.Text(manifest, "stage_id");
            string manifestSha = EvidenceIo.Sha256File(identity);
            ...
            List<Dictionary<string, object>> qaBefore = HarnessOps.QaOwnedInventory(c.Stage);
            List<Dictionary<string, object>> descendantsBefore = HarnessOps.DescendantInventory(self.Pid);
            if (forbiddenBefore.Count != 0) throw new HarnessException("Forbidden runtime existed at LIVE evidence contract self-test start");
            HarnessOps.RequireExactQaInventory(qaBefore, self);
            int allowedConsoleHostsBefore = HarnessOps.RequireExactDescendantInventory(descendantsBefore);

            Dictionary<string, object> sentinel;
            uint sentinelActive;
            using (JobScope sentinelJob = new JobScope("live-evidence-contract-selftest"))
            {
                sentinel = SentinelExercise.Run(
                    c,
                    sentinelJob,
                    "sentinel-live-evidence-contract-selftest",
                    self,
                    new ProcessIdentity[] { self },
                    new ProcessIdentity[0]);
                sentinelActive = sentinelJob.ActiveProcesses;
                if (sentinelActive != 0) throw new HarnessException("LIVE evidence contract sentinel retained an owned process");
            }

            ulong settleOrigin = NativeApi.GetTickCount64();
            c.Journal.Append("settle_origin_after_sentinel_exit", Record.Map(
                "settle_origin", settleOrigin.ToString(CultureInfo.InvariantCulture),
                "sentinel_exit_code", sentinel["exit_code"],
                "sentinel_owned_child_disposed", sentinel["owned_child_disposed"],
                "sentinel_remaining_job_processes", sentinel["remaining_job_processes"],
                "fixture", "qp_live_evidence_contract_selftest",
                "performance_slot_launch_count", 0));

            List<Dictionary<string, object>> qaAfterSentinel = HarnessOps.QaOwnedInventory(c.Stage);
            List<Dictionary<string, object>> descendantsAfterSentinel = HarnessOps.DescendantInventory(self.Pid);
            HarnessOps.RequireExactQaInventory(qaAfterSentinel, self);
            HarnessOps.RequireExactDescendantInventory(descendantsAfterSentinel);
            ulong sampleActual = NativeApi.GetTickCount64();
            Dictionary<string, object> positiveSample = HarnessOps.BuildLiveSample(
                0,
                settleOrigin,
                sampleActual,
                new List<Dictionary<string, object>>(),
                SyntheticHost(Contract.BestPerformanceGuid),
                new List<Dictionary<string, object>>(),
                qaAfterSentinel,
                descendantsAfterSentinel);
            Dictionary<string, object> positiveSamplePersistence = HarnessOps.PersistAndValidateLiveSample(c.Journal, positiveSample);
            Dictionary<string, object> sentinelOrder = VerifySentinelSampleOrder(c.JournalPath, "sentinel-live-evidence-contract-selftest");

            Dictionary<string, object> missingSlot = RunLiveSampleSlotNegativeFixture(c, positiveSample, "missing_slot", true);
            Dictionary<string, object> nonzeroSlot = RunLiveSampleSlotNegativeFixture(c, positiveSample, "nonzero_slot", false);
            Dictionary<string, object> evaluationFixtures = RunLiveEvaluationCompletenessFixtures(c, identity, stageId, manifestSha, receiptSha);
            List<Dictionary<string, object>> activationFixtures = RunActivationFixtures(c, identity, stageId, manifestSha, receiptSha);
            Dictionary<string, object> binding = AuditLiveEvidenceContractBinding(c, identity);
            Dictionary<string, object> sampleContract = Record.Map(
                "positive_sample_persistence", positiveSamplePersistence,
                "missing_slot_fixture", missingSlot,
                "nonzero_slot_fixture", nonzeroSlot);
            Dictionary<string, object> sentinelContract = Record.Map(
                "sentinel", sentinel,
                "sentinel_order", sentinelOrder);
            Dictionary<string, object> evaluationContract = Record.Map(
                "live_evaluation_fixtures", evaluationFixtures,
                "activation_fixture_results", activationFixtures);

            List<Dictionary<string, object>> forbiddenAfter = HarnessOps.ForbiddenRuntimeInventory(c.Stage);
            List<Dictionary<string, object>> qaAfter = HarnessOps.QaOwnedInventory(c.Stage);
            List<Dictionary<string, object>> descendantsAfter = HarnessOps.DescendantInventory(self.Pid);
            if (forbiddenAfter.Count != 0) throw new HarnessException("Forbidden runtime existed at LIVE evidence contract self-test end");
            HarnessOps.RequireExactQaInventory(qaAfter, self);
            int allowedConsoleHostsAfter = HarnessOps.RequireExactDescendantInventory(descendantsAfter);

            int caseCount = 9 + activationFixtures.Count;
            c.Journal.Append("qp_live_evidence_contract_selftest_complete", Record.Map(
                "case_count", caseCount,
                "passed_case_count", caseCount,
                "sentinel_exit_code", sentinel["exit_code"],
                ...
                "final_owned_runtime_count", 0));
            return Record.Map(
                "live_evidence_contract_selftest", "pass",
                "case_count", caseCount,
                "passed_case_count", caseCount,
                "sample_contract", sampleContract,
                "sentinel_contract", sentinelContract,
                ...
                "activation_fixture_results", activationFixtures,
                "production_binding_audit", binding,
                "allowed_windows_console_host_count_before", allowedConsoleHostsBefore,
                "allowed_windows_console_host_count_after", allowedConsoleHostsAfter,
                "controller_launch_count", 0,
                "performance_slot_launch_count", 0,
                "abc_launch_count", 0,
                "p95_produced", false,
                "kbm_performed", false,
                "final_owned_runtime_count", 0,
                "identity_audit", HarnessOps.FileSetAudit(identity, c.Stage));
        }

        private static Dictionary<string, object> RunLiveSampleSlotNegativeFixture(RoleContext c, Dictionary<string, object> positiveSample, string name, bool removeSlot)
        {
            string root = Path.Combine(c.Output, "sample-fixtures", name);
            Directory.CreateDirectory(root);
            string journalPath = Path.Combine(root, "evidence.journal.jsonl");
            EvidenceJournal journal = new EvidenceJournal(journalPath);
            Dictionary<string, object> sample = CloneMap(positiveSample);
            if (removeSlot) sample.Remove("performance_slot_launch_count");
            else sample["performance_slot_launch_count"] = 1;
            bool rejected = false;
            string rejection = null;
            try { HarnessOps.PersistAndValidateLiveSample(journal, sample); }
            catch (HarnessException expected) { rejected = true; rejection = expected.Message; }
            if (!rejected) throw new HarnessException("Invalid LIVE sample slot fixture was not rejected: " + name);
            Dictionary<string, object> verification = EvidenceJournal.VerifyFile(journalPath);
            string[] lines = File.ReadAllLines(journalPath, Encoding.UTF8);
            if (lines.Length != 1) throw new HarnessException("LIVE sample slot fixture journal record count mismatch: " + name);
            Dictionary<string, object> record = Record.AsMap(Record.Parse(lines[0]));
            Dictionary<string, object> payload = Record.AsMap(record["payload"]);
            object observedSlot;
            bool observedSlotPresent = payload.TryGetValue("performance_slot_launch_count", out observedSlot);
            if (removeSlot ? observedSlotPresent : (!observedSlotPresent || !(observedSlot is Int32) || ((Int32)observedSlot) != 1)) throw new HarnessException("LIVE sample slot fixture durable payload mismatch: " + name);
            Dictionary<string, object> result = Record.Map(
                "name", name,
                "expected_rejection", true,
                "rejected_after_durable_readback_and_hash", true,
                "rejection", rejection,
                "journal_path", journalPath,
                "journal_sha256", EvidenceIo.Sha256File(journalPath),
                "journal_verification", verification,
                "observed_slot_present", observedSlotPresent,
                "observed_slot_value", observedSlotPresent ? observedSlot : null,
                "passed", true);
            string resultPath = Path.Combine(root, "case-result.json");
            EvidenceIo.WriteNewJson(resultPath, result);
            return Record.Map("path", resultPath, "sha256", EvidenceIo.Sha256File(resultPath), "result", result);
        }

        private static Dictionary<string, object> VerifySentinelSampleOrder(string journalPath, string sentinelLabel)
        {
            string[] lines = File.ReadAllLines(journalPath, Encoding.UTF8);
            int ownedExitSequence = -1;
            int sentinelCompleteSequence = -1;
            int settleSequence = -1;
            int sampleSequence = -1;
            Dictionary<string, object> sentinelCompletePayload = null;
            Dictionary<string, object> samplePayload = null;
            for (int i = 0; i < lines.Length; i++)
            {
                Dictionary<string, object> record = Record.AsMap(Record.Parse(lines[i]));
                string kind = Record.Text(record, "kind");
                Dictionary<string, object> payload = Record.AsMap(record["payload"]);
                if (String.Equals(kind, "owned_child_exit", StringComparison.Ordinal) && String.Equals(HarnessOps.SafeText(payload, "label"), sentinelLabel, StringComparison.Ordinal)) ownedExitSequence = Record.Integer(record, "sequence");
                else if (String.Equals(kind, "sentinel_complete", StringComparison.Ordinal) && String.Equals(HarnessOps.SafeText(payload, "label"), sentinelLabel, StringComparison.Ordinal))
                {
                    sentinelCompleteSequence = Record.Integer(record, "sequence");
                    sentinelCompletePayload = payload;
                }
                else if (String.Equals(kind, "settle_origin_after_sentinel_exit", StringComparison.Ordinal)) settleSequence = Record.Integer(record, "sequence");
                else if (String.Equals(kind, "live_sample", StringComparison.Ordinal) && HarnessOps.SafeInteger(payload, "n", -1) == 0)
                {
                    sampleSequence = Record.Integer(record, "sequence");
                    samplePayload = payload;
                }
            }
            if (!(ownedExitSequence > 0 && sentinelCompleteSequence == ownedExitSequence + 1 && settleSequence == sentinelCompleteSequence + 1 && sampleSequence == settleSequence + 1)) throw new HarnessException("Sentinel/sample durable event order was not contiguous and exact");
            if (sentinelCompletePayload == null || !HarnessOps.SafeBoolean(sentinelCompletePayload, "owned_child_disposed", false) || HarnessOps.SafeInteger(sentinelCompletePayload, "remaining_job_processes", -1) != 0) throw new HarnessException("Sentinel completion did not prove child disposal and job zero");
            object slotValue;
            if (samplePayload == null || !samplePayload.TryGetValue("performance_slot_launch_count", out slotValue) || !(slotValue is Int32) || ((Int32)slotValue) != 0) throw new HarnessException("Ordered n=0 sample lacked durable numeric slot zero");
            return Record.Map(
                "owned_child_exit_sequence", ownedExitSequence,
                "sentinel_complete_sequence", sentinelCompleteSequence,
                "settle_origin_sequence", settleSequence,
                "live_sample_n0_sequence", sampleSequence,
                "contiguous_exact_order", true,
                "owned_child_disposed", true,
                "remaining_job_processes", 0,
                "performance_slot_launch_count", 0,
                "journal_sha256", EvidenceIo.Sha256File(journalPath),
                "journal_verification", EvidenceJournal.VerifyFile(journalPath));
        }

        private static Dictionary<string, object> RunLiveEvaluationCompletenessFixtures(RoleContext c, string identity, string stageId, string manifestSha, string receiptSha)
        {
            string root = Path.Combine(c.Output, "live-evaluation-fixtures");
            Directory.CreateDirectory(root);
            ...
            Dictionary<string, object> runnerIdentityCapture = HarnessOps.SafeCapture("live_completeness_runner_identity", delegate { return (object)HarnessOps.SelfIdentity(); });
            Dictionary<string, object> runnerIdentity = HarnessOps.CaptureValueMap(runnerIdentityCapture);
            Dictionary<string, object> launcherIdentity = Record.Map(
                "pid", checked(Record.Integer(runnerIdentity, "pid") + 100000),
                "creation_filetime_utc", (UInt64.Parse(Record.Text(runnerIdentity, "creation_filetime_utc"), CultureInfo.InvariantCulture) + 1UL).ToString(CultureInfo.InvariantCulture),
                "image_path", Path.Combine(c.Stage, "bin", "MfoQaLauncher.exe"));
            Dictionary<string, object> launcherIdentityCapture = SuccessfulCapture("live_completeness_launcher_identity", launcherIdentity);
            Dictionary<string, object> runtimeCapture = SuccessfulCapture("live_completeness_runtime", Record.Map());
            Dictionary<string, object> oneDriveCapture = SuccessfulCapture("live_completeness_onedrive", Record.Map("count", 0, "processes", new object[0]));
            Dictionary<string, object> hostCapture = SuccessfulCapture("live_completeness_host", SyntheticHost(Contract.BestPerformanceGuid));
            Dictionary<string, object> runnerTickCapture = SuccessfulCapture("live_completeness_runner_tick", Record.Map("value", "200"));
            Dictionary<string, object> runnerCliCapture = SuccessfulCapture("live_completeness_runner_cli", Record.Map("role", "fixture", "mode", "LIVE", "identity", identity, "out", root, "journal", c.JournalPath, "result", c.ResultPath));

            string runnerPositiveRoot = Path.Combine(root, "runner-positive");
            Directory.CreateDirectory(runnerPositiveRoot);
            string runnerPositivePendingPath = Path.Combine(runnerPositiveRoot, "pending.json");
            string runnerPositiveEvaluationPath = Path.Combine(runnerPositiveRoot, "evaluation.json");
            Dictionary<string, object> runnerPositivePending = HarnessOps.BuildLiveActivationPendingObservation(
                runnerPositivePendingPath, c.Stage, identity, Path.Combine(c.Stage, "seal", "preparation-receipt.json"), Path.Combine(root, "unused-audit.json"), Path.Combine(root, "unused-preack.json"), Path.Combine(root, "unused-preack-evaluation.json"), activationPath,
                stageId, manifestSha, receiptSha, auditSha, preackSha, preackEvaluationSha, preackTick.ToString(CultureInfo.InvariantCulture), "synthetic runner completeness positive",
                runnerIdentityCapture, runtimeCapture, oneDriveCapture, hostCapture, runnerTickCapture, runnerCliCapture);
            Dictionary<string, object> runnerPositivePersisted = HarnessOps.PersistCompleteActivationObservation(c.Journal, runnerPositivePendingPath, runnerPositivePending);
            Dictionary<string, object> runnerPositiveEvaluation = HarnessOps.EvaluatePersistedActivationObservation(
                c.Journal, runnerPositiveEvaluationPath, runnerPositivePersisted, c.Stage, identity, Path.Combine(c.Stage, "seal", "preparation-receipt.json"), Path.Combine(root, "unused-audit.json"), Path.Combine(root, "unused-preack.json"), Path.Combine(root, "unused-preack-evaluation.json"), activationPath,
                stageId, manifestSha, receiptSha, auditSha, preackSha, preackEvaluationSha, preackTick.ToString(CultureInfo.InvariantCulture), false);
            Dictionary<string, object> runnerPositiveResult = RequireLiveEvaluationCompleteness(runnerPositiveEvaluation, true, Contract.Pass, "runner-positive");

            string runnerMissingRoot = Path.Combine(root, "runner-missing-required-field");
            Directory.CreateDirectory(runnerMissingRoot);
            string runnerMissingPendingPath = Path.Combine(runnerMissingRoot, "pending.json");
            string runnerMissingEvaluationPath = Path.Combine(runnerMissingRoot, "evaluation.json");
            Dictionary<string, object> runnerMissingPending = CloneMap(runnerPositivePending);
            runnerMissingPending["pending_observation_path"] = Path.GetFullPath(runnerMissingPendingPath);
            runnerMissingPending.Remove("preparation_audit_capture");
            Dictionary<string, object> runnerMissingPersisted = HarnessOps.PersistObservationReadback(c.Journal, "live_activation_pending_observation", runnerMissingPendingPath, runnerMissingPending);
            Dictionary<string, object> runnerMissingEvaluation = HarnessOps.EvaluatePersistedActivationObservation(
                c.Journal, runnerMissingEvaluationPath, runnerMissingPersisted, c.Stage, identity, Path.Combine(c.Stage, "seal", "preparation-receipt.json"), Path.Combine(root, "unused-audit.json"), Path.Combine(root, "unused-preack.json"), Path.Combine(root, "unused-preack-evaluation.json"), activationPath,
                stageId, manifestSha, receiptSha, auditSha, preackSha, preackEvaluationSha, preackTick.ToString(CultureInfo.InvariantCulture), false);
            Dictionary<string, object> runnerMissingResult = RequireLiveEvaluationCompleteness(runnerMissingEvaluation, false, Contract.HarnessFail, "runner-missing-required-field");

            string activationEvaluationPath = Path.Combine(root, "runner-activation-evaluation-fixture.json");
            EvidenceIo.WriteNewJson(activationEvaluationPath, Record.Map("schema", Contract.ActivationEvaluationSchema, "result_code", Contract.Pass, "exact_activation_match", true, "binding_match", true, "pending_readback_success", true, "pending_field_completeness_success", true));
            ...
            Dictionary<string, object> launcherTickCapture = SuccessfulCapture("live_completeness_launcher_tick", Record.Map("value", "201"));
            Dictionary<string, object> launcherCliCapture = SuccessfulCapture("live_completeness_launcher_cli", Record.Map("role", "fixture", "mode", "LIVE", "identity", identity, "out", root, "journal", c.JournalPath, "result", c.ResultPath));

            string launcherPositiveRoot = Path.Combine(root, "launcher-positive");
            Directory.CreateDirectory(launcherPositiveRoot);
            string launcherPositivePendingPath = Path.Combine(launcherPositiveRoot, "pending.json");
            string launcherPositiveEvaluationPath = Path.Combine(launcherPositiveRoot, "evaluation.json");
            Dictionary<string, object> launcherPositivePending = HarnessOps.BuildLauncherLivePendingObservation(c, identity, launcherPositivePendingPath, fields, runnerIdentityCapture, launcherIdentityCapture, runtimeCapture, launcherTickCapture, launcherCliCapture);
            Dictionary<string, object> launcherPositivePersisted = HarnessOps.PersistCompleteLauncherLiveObservation(c.Journal, launcherPositivePendingPath, launcherPositivePending);
            Dictionary<string, object> launcherPositiveEvaluation = HarnessOps.EvaluatePersistedLauncherLiveObservation(c.Journal, launcherPositiveEvaluationPath, launcherPositivePersisted, c, identity, fields, false);
            Dictionary<string, object> launcherPositiveResult = RequireLiveEvaluationCompleteness(launcherPositiveEvaluation, true, Contract.Pass, "launcher-positive");

            string launcherMissingRoot = Path.Combine(root, "launcher-missing-required-field");
            Directory.CreateDirectory(launcherMissingRoot);
            string launcherMissingPendingPath = Path.Combine(launcherMissingRoot, "pending.json");
            string launcherMissingEvaluationPath = Path.Combine(launcherMissingRoot, "evaluation.json");
            Dictionary<string, object> launcherMissingPending = CloneMap(launcherPositivePending);
            launcherMissingPending["pending_observation_path"] = Path.GetFullPath(launcherMissingPendingPath);
            launcherMissingPending.Remove("preparation_audit_capture");
            Dictionary<string, object> launcherMissingPersisted = HarnessOps.PersistObservationReadback(c.Journal, "launcher_live_pending_observation", launcherMissingPendingPath, launcherMissingPending);
            Dictionary<string, object> launcherMissingEvaluation = HarnessOps.EvaluatePersistedLauncherLiveObservation(c.Journal, launcherMissingEvaluationPath, launcherMissingPersisted, c, identity, fields, false);
            Dictionary<string, object> launcherMissingResult = RequireLiveEvaluationCompleteness(launcherMissingEvaluation, false, Contract.HarnessFail, "launcher-missing-required-field");

            return Record.Map(
                "runner_positive", runnerPositiveResult,
                "runner_missing_required_field", runnerMissingResult,
                "launcher_positive", launcherPositiveResult,
                "launcher_missing_required_field", launcherMissingResult,
                "positive_count", 2,
        ...
        private static Dictionary<string, object> RequireLiveEvaluationCompleteness(Dictionary<string, object> evaluationReference, bool expectedComplete, int expectedCode, string label)
        {
            string path = Convert.ToString(evaluationReference["evaluation_path"], CultureInfo.InvariantCulture);
            Dictionary<string, object> evaluation = EvidenceIo.ReadMap(path);
            bool readback = HarnessOps.SafeBoolean(evaluation, "pending_readback_success", false);
            bool complete = HarnessOps.SafeBoolean(evaluation, "pending_field_completeness_success", !expectedComplete);
            int code = HarnessOps.SafeInteger(evaluation, "result_code", -1);
            if (!readback || complete != expectedComplete || code != expectedCode) throw new HarnessException("LIVE evaluation completeness fixture mismatch: " + label);
            return Record.Map(
                "label", label,
                "path", path,
                "sha256", EvidenceIo.Sha256File(path),
                "pending_readback_success", readback,
                "pending_field_completeness_success", complete,
        ...
        private static Dictionary<string, object> AuditLiveEvidenceContractBinding(RoleContext c, string identity)
        {
            string sourcePath = Path.Combine(c.Stage, "source", "MfoQaNative.cs");
            string source = File.ReadAllText(sourcePath, Encoding.UTF8);
            string captureStartMarker = "        private static ulong " + "CaptureLiveSample(RoleContext c";
            int captureStart = source.IndexOf(captureStartMarker, StringComparison.Ordinal);
            int captureEnd = source.Length;
            if (captureStart < 0 || captureEnd <= captureStart) throw new HarnessException("Production live sample source boundary missing");
            string capture = source.Substring(captureStart, captureEnd - captureStart);
            string builderCall = "HarnessOps.BuildLive" + "Sample";
            string persistCall = "HarnessOps.PersistAndValidateLive" + "Sample";
            int build = capture.IndexOf(builderCall, StringComparison.Ordinal);
            int persist = capture.IndexOf(persistCall, StringComparison.Ordinal);
            if (!(build >= 0 && build < persist)) throw new HarnessException("Production live sample builder/persistence binding failed");

            string controllerStartMarker = "        private static Dictionary<string, object> " + "Live(RoleContext c, ulong origin, ulong deadline, string identity)";
            int controllerStart = source.IndexOf(controllerStartMarker, StringComparison.Ordinal);
            int controllerEnd = source.IndexOf(captureStartMarker, controllerStart + controllerStartMarker.Length, StringComparison.Ordinal);
            if (controllerStart < 0 || controllerEnd <= controllerStart) throw new HarnessException("Production controller LIVE source boundary missing");
            string controller = source.Substring(controllerStart, controllerEnd - controllerStart);
            int sentinel = controller.IndexOf("SentinelExercise." + "Run", StringComparison.Ordinal);
            int settle = controller.IndexOf("settle_origin_after_sentinel_" + "exit", StringComparison.Ordinal);
            int n0 = controller.IndexOf("CaptureLive" + "Sample(c, 0", StringComparison.Ordinal);
            if (!(sentinel >= 0 && sentinel < settle && settle < n0) || controller.IndexOf("delegate(ulong exitObserved" + "Tick)", StringComparison.Ordinal) >= 0) throw new HarnessException("Production sentinel/settle/n0 ordering binding failed");

            string sentinelStartMarker = "    public static class " + "SentinelExercise";
            string sentinelEndMarker = "    public static class " + "RunnerRole";
            int sentinelStart = source.IndexOf(sentinelStartMarker, StringComparison.Ordinal);
            int sentinelEnd = source.IndexOf(sentinelEndMarker, sentinelStart + sentinelStartMarker.Length, StringComparison.Ordinal);
            if (sentinelStart < 0 || sentinelEnd <= sentinelStart) throw new HarnessException("Sentinel production source boundary missing");
            string sentinelSource = source.Substring(sentinelStart, sentinelEnd - sentinelStart);
            int wait = sentinelSource.IndexOf("child.WaitUntil(origin, deadline, owner)", StringComparison.Ordinal);
            int disposed = sentinelSource.IndexOf("uint remainingJobProcesses = owner.Active" + "Processes", StringComparison.Ordinal);
            int complete = sentinelSource.IndexOf("sentinel_" + "complete", StringComparison.Ordinal);
            if (!(wait >= 0 && wait < disposed && disposed < complete) || sentinelSource.IndexOf("onExit" + "Observed", StringComparison.Ordinal) >= 0) throw new HarnessException("Sentinel cleanup production binding failed");

            string runnerEvaluatorStart = "        public static Dictionary<string, object> " + "EvaluatePersistedActivationObservation(";
            string launcherBuilderStart = "        public static Dictionary<string, object> " + "BuildLauncherLivePendingObservation(";
            int runnerEvaluator = source.IndexOf(runnerEvaluatorStart, StringComparison.Ordinal);
            int launcherBuilder = source.IndexOf(launcherBuilderStart, runnerEvaluator + runnerEvaluatorStart.Length, StringComparison.Ordinal);
            string runnerEvaluatorSource = source.Substring(runnerEvaluator, launcherBuilder - runnerEvaluator);
            string launcherEvaluatorStart = "        public static Dictionary<string, object> " + "EvaluatePersistedLauncherLiveObservation(";
            string evaluatorEnd = "        private static Dictionary<string, object> " + "SuccessfulRawCapture(";
            int launcherEvaluator = source.IndexOf(launcherEvaluatorStart, StringComparison.Ordinal);
            int launcherEvaluatorEnd = source.IndexOf(evaluatorEnd, launcherEvaluator + launcherEvaluatorStart.Length, StringComparison.Ordinal);
            string launcherEvaluatorSource = source.Substring(launcherEvaluator, launcherEvaluatorEnd - launcherEvaluator);
            string completenessField = "pending_field_completeness_" + "success";
            if (runnerEvaluatorSource.IndexOf("HasCompleteLiveActivationPending" + "FieldSet", StringComparison.Ordinal) < 0 || runnerEvaluatorSource.IndexOf(completenessField, StringComparison.Ordinal) < 0 || runnerEvaluatorSource.IndexOf("readback && complete", StringComparison.Ordinal) < 0) throw new HarnessException("Runner LIVE evaluator completeness binding failed");
            if (launcherEvaluatorSource.IndexOf("HasCompleteLauncherLivePending" + "FieldSet", StringComparison.Ordinal) < 0 || launcherEvaluatorSource.IndexOf(completenessField, StringComparison.Ordinal) < 0 || launcherEvaluatorSource.IndexOf("readback && complete", StringComparison.Ordinal) < 0) throw new HarnessException("Launcher LIVE evaluator completeness binding failed");

            string qpStartMarker = "        private static Dictionary<string, object> " + "RunLiveEvidenceContractSelfTest(RoleContext c, string identity)";
            string qpEndMarker = "        private static Dictionary<string, object> " + "RunLiveSampleSlotNegativeFixture(";
            int qpStart = source.IndexOf(qpStartMarker, StringComparison.Ordinal);
            int qpEnd = source.IndexOf(qpEndMarker, qpStart + qpStartMarker.Length, StringComparison.Ordinal);
            string qpSource = source.Substring(qpStart, qpEnd - qpStart);
            if (qpSource.IndexOf("Run" + "Child(", StringComparison.Ordinal) >= 0 || qpSource.IndexOf("Start" + "Role(", StringComparison.Ordinal) >= 0) throw new HarnessException("LIVE evidence contract self-test could start launcher/controller");
            if (!String.Equals(Contract.WorkOrder, "MFO-WO-P2-2A-" + "009", StringComparison.Ordinal)) throw new HarnessException("Production work-order identity did not roll to -009");
            if (source.IndexOf("MFO-WO-P2-2A-" + "006", StringComparison.Ordinal) >= 0 || source.IndexOf("MFO-WO-P2-2A-" + "007", StringComparison.Ordinal) >= 0 || source.IndexOf("MFO-WO-P2-2A-" + "008", StringComparison.Ordinal) >= 0) throw new HarnessException("Legacy work-order literal escaped named negative fixtures");
            return Record.Map(
                "source_path", sourcePath,
                "source_sha256", EvidenceIo.Sha256File(sourcePath),
                "manifest_path", identity,
                "manifest_sha256", EvidenceIo.Sha256File(identity),
                "production_sample_builder", "HarnessOps.BuildLiveSample",
                "production_sample_persistence", "HarnessOps.PersistAndValidateLiveSample",
                "production_sentinel_sequence", "SentinelExercise.Run",
                "production_runner_evaluator", "HarnessOps.EvaluatePersistedActivationObservation",
                "production_launcher_evaluator", "HarnessOps.EvaluatePersistedLauncherLiveObservation",
                "sample_builder_binding_pass", true,
                "sentinel_order_binding_pass", true,
                "runner_evaluation_completeness_binding_pass", true,
                "launcher_evaluation_completeness_binding_pass", true,
                "runner_only_qp_no_child_start_pass", true,
                "exact_work_order_009_pass", true,
                "legacy_production_literal_count", 0,
                "performance_slot_launch_count", 0,
                "controller_launch_count", 0);
        }
        ...
        private static List<Dictionary<string, object>> RunActivationFixtures(RoleContext c, string identity, string stageId, string manifestSha, string receiptSha)
        {
            ...
            fixtures.Add(new KeyValuePair<string, string>("old_008", ("MFO-WO-P2-2A-" + "008") + exact.Substring(Contract.WorkOrder.Length)));
            ...
            fixtures.Add(new KeyValuePair<string, string>("extra_cr", exact + "\r"));
            fixtures.Add(new KeyValuePair<string, string>("extra_lf", exact + "\n"));
            fixtures.Add(new KeyValuePair<string, string>("extra_crlf", exact + "\r\n"));
        ...
        private static Dictionary<string, object> AuditPreackContractBinding(RoleContext c, string identity)
        {
            ...
            if (source.IndexOf("MFO-WO-P2-2A-" + "006", StringComparison.Ordinal) >= 0 || source.IndexOf("MFO-WO-P2-2A-" + "007", StringComparison.Ordinal) >= 0 || source.IndexOf("MFO-WO-P2-2A-" + "008", StringComparison.Ordinal) >= 0) throw new HarnessException("Stale activation work-order literal remained outside named negative fixtures");
    ...
    public static class ControllerRole
    {
        ...
        private static Dictionary<string, object> SelfTest(RoleContext c, ulong origin, ulong deadline)
        {
            ...
            using (JobScope sentinelJob = new JobScope("sentinel-selftest"))
            {
                sentinel = SentinelExercise.Run(
                    ...
                    "sentinel-selftest",
                    runner,
                    new ProcessIdentity[] { runner, launcher, self },
                    new ProcessIdentity[] { launcher, self });
            ...
            c.Journal.Append("qp_selftest_complete", Record.Map("allowed_windows_console_host_count", allowedWindowsConsoleHostCount, "unexpected_descendant_count", 0, "performance_slot_launch_count", 0, "abc_launch_count", 0, "final_owned_runtime_count", active));
            return Record.Map("native_origin", origin.ToString(CultureInfo.InvariantCulture), "deadline", deadline.ToString(CultureInfo.InvariantCulture), "synthetic_expiry", "pass", "synthetic_inventory_negative", "pass", "sentinel", sentinel, "qa_owned_processes", qaInventory, "runner_descendants", descendants, "allowed_windows_console_host_count", allowedWindowsConsoleHostCount, "unexpected_descendant_count", 0, "performance_slot_launch_count", 0, "abc_launch_count", 0, "final_owned_runtime_count", active);
        }
        ...
        private static Dictionary<string, object> Live(RoleContext c, ulong origin, ulong deadline, string identity)
        {
            ...
            using (JobScope sentinelJob = new JobScope("sentinel-live"))
            {
                sentinel = SentinelExercise.Run(
                    ...
                    "sentinel-live",
                    runnerIdentity,
                    new ProcessIdentity[] { runnerIdentity, launcherIdentity, selfIdentity },
                    new ProcessIdentity[] { launcherIdentity, selfIdentity });
        ...
        private static ulong CaptureLiveSample(RoleContext c, int n, ulong target, ulong deadline, uint baselineInput, ulong previous, ProcessIdentity runnerIdentity, ProcessIdentity launcherIdentity, ProcessIdentity selfIdentity)
        {
            ...
            Dictionary<string, object> sample = HarnessOps.BuildLiveSample(n, target, actual, oneDrive, host, forbidden, qaInventory, descendants);
            HarnessOps.PersistAndValidateLiveSample(c.Journal, sample);
