...
namespace MfoQaQualification
{
    ...
    public static class NativeApi
    {
        ...
        private static extern uint PowerGetEffectiveOverlayScheme(out IntPtr effectivePowerModeGuid);
        ...
        public static string EffectiveOverlayGuid(out bool success, out uint nativeStatus)
        {
            ...
            nativeStatus = PowerGetEffectiveOverlayScheme(out pointer);
            success = nativeStatus == 0 && pointer != IntPtr.Zero;
            if (!success) return String.Empty;
            try { return Marshal.PtrToStructure(pointer, typeof(Guid)).ToString().ToLowerInvariant(); }
    ...
    public static class HarnessOps
    {
        ...
        public static void ValidateRolePaths(string role, string mode, string stage, string identity, string output, string journal, string result)
        {
            ...
            string expectedIdentity = (mode == "QP_DRYRUN" || mode == "QP_SELFTEST") ? Path.Combine(stage, "config", "component-contract.json") : Path.Combine(stage, "seal", "qualification-manifest.json");
