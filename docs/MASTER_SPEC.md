...
entry P1は承認済み。`MFO-WO-P2-2A-001`、`-002`、`-003`は返却済みで、現在は
`MFO-WO-P2-2A-004`のQA-only controlled rerunを発行済みだが、ユーザーのquiet-window確認までtimed
matrixを開始しない。ゲームコード変更は許可しない。2-B以降、損傷、HUD統合、素材、boss、stageは
別work orderまでlockする。詳細は
