...
entry P1は承認済み。`MFO-WO-P2-2A-001`から`-009`は返却済みで、Slice 2-Aはfunctional checksと
...
記録したがABI不一致でFail、`-007`はPREACK contract不適合でFail、`-008`はPREACK／exact activation／
61-sample LIVE後にper-sample slot field、sentinel cleanup順、LIVE evaluation completenessの3契約不適合で
Failとなった。`MFO-WO-P2-2A-009`はこの3件を限定修正し、5-mode preparation、PREACK、exact activation、
