...
entry P1は承認済み。`MFO-WO-P2-2A-001`から`-006`は返却済みで、Slice 2-Aはfunctional checksと
...
`0`のまま終了した。現在のactive scopeは`MFO-WO-P2-2A-007`によるABI限定修正と新stageでのQA harness
再資格確認だけであり、再測定もゲームコード変更も許可しない。再資格確認Passでも測定票は自動発行しない。
