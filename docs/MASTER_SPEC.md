...
entry P1は承認済み。`MFO-WO-P2-2A-001`から`-004`は返却済みで、Slice 2-Aはfunctional checksと
corrected-C KBMがPassである。correction performance Failは保持し、`-003`／`-004`のcontrolled matrixは
valid run `0`で、現在のperformance acceptanceは未解決である。現在は`MFO-WO-P2-2A-005`のQA-only
performance再検証を発行済みで、OneDrive外のpreparationをsealした後、userが全OneDrive clientを終了した
quiet windowだけで固定matrixを実行する。ゲームコード変更は許可しない。2-B以降、損傷、HUD統合、素材、
boss、stageは別work orderまでlockする。詳細は
