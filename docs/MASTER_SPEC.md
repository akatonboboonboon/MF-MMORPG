...
entry P1は承認済み。`MFO-WO-P2-2A-001`から`-007`は返却済みで、Slice 2-Aはfunctional checksと
...
`Fail / harness defect`となった。現在のactive scopeは`MFO-WO-P2-2A-008`によるこの3件の限定修正、seal前
contract test、新stageでのQA harness再資格確認だけであり、再測定もゲームコード変更も許可しない。
