...
- Gate 0: Open
- Gate 1: Pending power-mode revalidation and QA recommendation
...
`バランス`、AC／DC power mode `961cc777-2547-4f9d-8174-7d86181b8a7a`である。これは
Best Power Efficiency相当で承認済み性能優先に不適合なため、Best performanceでGate 1性能を再計測する。
...
回避無敵、スタミナ、キャンセル、部位ロック、自動接近はOD-020未決定。固定方角は承認済みだが、
ズーム、画面内人数、ボスの最大表示寸法はOD-041未決定である。
...
- 変形閾値超過時のペナルティは1種類だけ。対象・閾値・効果量はOD-027未決定。
...
Phase 1の自動技術ベースラインは完了している。KBM実動作と総合体感はGate 1証拠へ採用し、物理gamepad
証拠はGate Playabilityまで延期した。Phase 1工数は承認済み再見積りで13.83%となる。Gate 1は
Best performance条件での性能再計測、QA勧告、監督の最終差分確認が完了するまで承認しない。詳細は
