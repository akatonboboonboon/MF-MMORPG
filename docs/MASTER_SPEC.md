...
entry P1は承認済み。`MFO-WO-P2-2A-001`から`-008`は返却済みで、Slice 2-Aはfunctional checksと
...
acceptanceに対して継続する。material host change報告後の`MFO-WO-P2-2A-006`はfresh OneDrive count `0`を
記録したが、effective-overlay APIのABI不一致によりPREACKで`Fail / harness defect`となり、performance slotは
`0`のまま終了した。`MFO-WO-P2-2A-007`はABI修正とproduction-path smokeを通したが、receipt identity、正しい
work-order token、complete PREACK recordのpersist-before-assert契約に不適合で、PREACK未実施／slot `0`の
`Fail / harness defect`となった。`MFO-WO-P2-2A-008`はPREACK、exact activation、61-sample LIVEまで完走したが、
per-sample slot field、sentinel cleanup順、LIVE evaluation completenessの3契約に不適合で、slot `0`の
`Fail / harness defect`となった。現在のactive scopeは`MFO-WO-P2-2A-009`によるこの3件のLIVE-evidence限定修正、
seal前contract test、fresh stageでのQA harness再資格確認だけであり、再測定もゲームコード変更も許可しない。
再資格確認Passでも測定票は自動発行しない。
