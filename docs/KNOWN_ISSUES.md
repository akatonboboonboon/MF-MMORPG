...
|---|---|---|---|---|---|---|
| KI-001 | Manual validation gap | Gate blocker | 実ゲームパッドの左stick、右stick、X入力を未確認 | 実機で操作し、入力結果と操作感を記録 | QA + user | Open |
| KI-002 | Evidence gap | Gate blocker | Phase 1工数が試作全体の15%以下である証跡がない | 実績または再見積りと全体見積りの単位を記録 | Supervisor | Open |
| KI-003 | Environment metadata | Gate blocker | Windows電源設定の実表示名とGPU driverが未記録 | 同じ基準端末で取得し、Gate 1証拠へ追加 | QA | Open |
