...
| KI-010 | Performance acceptance failure / `MFO-P2-2A-QA-002` | P1 acceptance blocker / runtime severity undetermined | correction QAのvalid 2 runはP95 `33.4643 ms`／`20.0000 ms`で`<= 16.67 ms`を超過。`-003`、`-004`、`-005`はいずれもvalid matrix `0`で、現在candidateの性能受入と因果は未解決 | `-009`でnon-performance harness qualificationはPassしたが性能測定は未実施。`MFO-HOLD-P2-2A-001`を維持し、別の明示performance work orderを待つ | `30 QA` + Supervisor + user | Open / harness qualified; performance hold active |
