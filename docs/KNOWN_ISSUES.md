...
| KI-003 | Environment nonconformance | Gate blocker | plan=`バランス`、AC/DC power mode=`Best Power Efficiency`でOD-004の性能優先に不適合。driverは`32.0.101.7077` | AC Best performanceへ変更し、empty／arenaを再計測 | `30 QA` + user | Confirmed / revalidation open |
...
- Phase 1の旧空／アリーナidle P95は履歴上の目標内記録だが、power modeが未記録のため現在のOD-004
  Gate 1証拠には流用しない。最大負荷、Gate 7、製品最低環境の保証でもない。
