...
| M2 / Phase 2 | 共通戦闘システム | Slice 2-A functional checks Pass / correction performance Fail retained / diagnostic Blocked / controlled rerun awaiting user | Gate 2 locked / not evaluated |
...
Phase 2 entry P1は承認済みだが、現在のactive票はSlice 2-AのQA-only controlled rerunだけで、ゲームコード変更は許可しない。

現在の実行順:

```text
00 accepted MFO-WO-P2-2A-003 Blocked with 0 valid acceptance runs
→ 00 issued MFO-WO-P2-2A-004, awaiting explicit user quiet-window confirmation
→ user + 30 run quiet-host performance, then user-operated corrected-C KBM
...
Active controlled-rerun order — timed matrix awaits user confirmation:
...
Status: **Functional correction verified / correction performance Fail retained / diagnostic Blocked / controlled rerun awaiting user**
...
- KBM: `Blocked / Not completed`; user feel Not run; physical gamepad Not run / Deferred
...
`30`は`MFO-WO-P2-2A-004`に従うが、ユーザーがOneDrive clientの手動pause／exitと端末無操作時間を
明示確認するまでtimed matrixを開始しない。quota整理／削除／同期成功は要求せず、確認後はOneDrive外
stagingで固定matrixを実施し、EXEをrepoへ戻さず、性能終了後にuser本人が
corrected-C KBMを操作する。`10`のgame code変更は原因範囲が証拠で確定するまで許可しない。
