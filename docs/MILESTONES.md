...
と`-007`は`Fail / harness defect`で返却済みで、現在のactive work orderは3件のharness契約限定修正と再資格確認専用
`MFO-WO-P2-2A-008`である。ゲームコード変更は許可しない。
...
→ 30 corrects only receipt binding, exact activation ID, and persist-before-assert ordering, then requalifies a new stage with performance slot count 0
...
揃えたscope／pathだけに発生する。現在はperformance holdとcontract-correction-requalification-only `-008`がactiveで、
...
Active harness contract correction／requalification order:
...
- `MFO-WO-P2-2A-008`: active three-contract-correction／new-stage requalification order; seal-before receipt／token／
  persist-order tests are mandatory, and performance slot count remains `0`
...
- `MFO-P2-2A-QA-004`: QA PREACK／activation contract defect; game defectではなく`MFO-WO-P2-2A-008`で限定修正中
...
`-007` Failを受理しstageを凍結したうえで、明示票`MFO-WO-P2-2A-008`を発行した。`30`は3件の限定修正、
seal前contract test、新stageのharness再資格確認だけを行い、performance slot、KBM、旧stage再利用は禁止する。
