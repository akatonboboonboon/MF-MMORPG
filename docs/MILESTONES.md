...
| M2 / Phase 2 | 共通戦闘システム | Slice 2-A correction QA Fail / functional checks Pass / performance diagnostic active / KBM Blocked | Gate 2 locked / not evaluated |
...
Phase 2 entry P1は承認済みだが、現在のactive票はSlice 2-AのQA診断だけで、ゲームコード変更は許可しない。

現在の実行順:

```text
00 accepted correction QA Fail and verified the functional correction
→ 00 issued MFO-WO-P2-2A-003
→ 30 isolates performance in one controlled session and completes KBM evidence
...
Status: **Functional correction verified / performance Fail / KBM Blocked / QA diagnostic active**
...
`30`だけが`MFO-WO-P2-2A-003`に従い、Gate 1／pre-correction／corrected buildを同一sessionで比較し、
corrected-release KBM checklistを完遂する。`10`のgame code変更は原因範囲が証拠で確定するまで許可しない。
