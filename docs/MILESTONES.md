...
| M2 / Phase 2 | 共通戦闘システム | Slice 2-A authorized / implementation pending | Gate 2 locked / not evaluated |
...
→ 00 issued MFO-WO-P2-2A-001
→ 10 implements Slice 2-A only
→ 30 validates Slice 2-A
...
Active work order: [`MFO-WO-P2-2A-001`](work-orders/phase2-slice2a-basic-operation.md)
...
Status: **Active / implementation authorized to `10` under MFO-WO-P2-2A-001**

- player移動、照準、回避
- lock-onはOD-020によりPhase 2対象外
- retry用authority reset seam。defeat／input bindingは実装しない

完了後に`30`が移動、回避、authority reset seamを検証する。
