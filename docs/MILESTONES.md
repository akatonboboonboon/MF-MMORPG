...
| M2 / Phase 2 | 共通戦闘システム | Functional checks + KBM Pass / correction performance Fail retained / valid matrix 0 / harness ABI correction and requalification active under performance hold | Gate 2 locked / not evaluated |
...
は`Fail / harness defect`で返却済みで、現在のactive work orderはABI限定修正とQA harness再資格確認専用
`MFO-WO-P2-2A-007`である。ゲームコード変更は許可しない。
...
→ 30 corrects only the effective-overlay ABI and requalifies a new stage with performance slot count 0
...
揃えたscope／pathだけに発生する。現在はperformance holdとcorrection-requalification-only `-007`がactiveで、
...
Active harness correction／requalification order:
...
Status: **Functional correction + corrected-C KBM verified / correction performance Fail retained / controlled matrices valid run 0 / harness ABI correction and requalification active under performance hold**
...
- `MFO-WO-P2-2A-007`: active ABI correction／new-stage requalification order; exact production `PowerAndInput` smoke
  must pass before seal, and performance slot count remains `0`
...
- `MFO-P2-2A-QA-003`: QA harness ABI defect; game defectではなく`MFO-WO-P2-2A-007`で限定修正中
...
そのFailを監督受理し旧stageを凍結したうえで、明示票`MFO-WO-P2-2A-007`を発行した。`30`はABI限定修正と
新stageのharness再資格確認だけを行い、performance slot、KBM、旧stage再利用は禁止する。`10`はgame code／
値／profiling seamを変更しない。再資格確認後は必ず`00`へ戻し、performance測定は別票まで行わない。
