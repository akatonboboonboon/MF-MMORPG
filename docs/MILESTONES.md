...
| M2 / Phase 2 | 共通戦闘システム | Functional checks + KBM Pass / correction performance Fail retained / valid matrix 0 / harness contract correction and requalification active under performance hold | Gate 2 locked / not evaluated |
...
から`-008`は`Fail / harness defect`で返却済みで、現在のactive work orderは3件のLIVE-evidence限定修正と再資格確認専用
`MFO-WO-P2-2A-009`である。ゲームコード変更は許可しない。
...
→ 30 corrects only per-sample slot evidence, sentinel cleanup ordering, and LIVE evaluation completeness, then requalifies a fresh stage with performance slot count 0
→ 00 reviews the result; 10 remains stopped and 20 remains non-binding-only
```
...
揃えたscope／pathだけに発生する。現在はperformance holdとLIVE-evidence-correction-requalification-only `-009`がactiveで、
work order／実装許可を持つgame code pathはない。
...
Active performance hold — only the explicit qualification exception below is authorized:
[`MFO-HOLD-P2-2A-001`](work-orders/phase2-slice2a-performance-external-hold.md)

Active harness LIVE-evidence correction／requalification order:
...
Status: **Functional correction + corrected-C KBM verified / correction performance Fail retained / controlled matrices valid run 0 / harness contract correction and requalification active under performance hold**
...
- `MFO-WO-P2-2A-009`: active three-LIVE-evidence-correction／fresh-stage requalification order; performance slot count remains `0`
...
- `MFO-P2-2A-QA-005`／`006`／`007`: LIVE sample slot field、sentinel cleanup順、evaluation completenessの
  harness defects; game defectではなく`MFO-WO-P2-2A-009`で限定修正中
...
そのFailを監督受理して`-007`を発行したが、PREACK開始前の契約監査で3件のharness defectが確定した。監督は
`-007` Failを受理しstageを凍結したうえで、明示票`MFO-WO-P2-2A-008`を発行した。`-008`はhost-stableなLIVEを
完走したが3件のLIVE evidence／ordering defectでFailとなった。監督はstage／runtime evidenceを凍結し、明示票
`MFO-WO-P2-2A-009`を発行した。`30`は3件の限定修正、seal前contract test、fresh stageのharness再資格確認だけを
行い、performance slot、KBM、旧stage再利用は禁止する。
`10`はgame code／値／profiling seamを変更しない。再資格確認後は必ず`00`へ戻し、performance測定は別票まで
行わない。
