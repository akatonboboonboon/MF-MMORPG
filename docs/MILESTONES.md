...
| M2 / Phase 2 | 共通戦闘システム | Functional checks + KBM Pass / correction performance Fail retained / valid matrix 0 / harness qualification active under performance hold | Gate 2 locked / not evaluated |
...
Phase 2 entry P1は承認済みだが、performance acceptanceは`MFO-HOLD-P2-2A-001`中である。現在のactive
work orderはQA harness資格確認専用`MFO-WO-P2-2A-006`で、ゲームコード変更は許可しない。
...
→ 00 explicitly issued MFO-WO-P2-2A-006 for harness qualification only
→ 30 returns Pass / Fail / Blocked with performance slot count 0
...
揃えたscope／pathだけに発生する。現在はperformance holdとqualification-only `-006`がactiveで、
...
Active harness-qualification order:
...
Status: **Functional correction + corrected-C KBM verified / correction performance Fail retained / controlled matrices valid run 0 / harness qualification active under performance hold**
...
- QA harness: auxiliary TickCount64 field `0` on unsupported PowerShell property; actual deadline used nonzero
  Stopwatch origin; current `MFO-WO-P2-2A-006` qualification addresses the remaining evidence-integrity requirements
...
- `MFO-WO-P2-2A-006`: active qualification-only order; new stage, native monotonic source, persisted pre-ack／trigger
  inventories, child exit／raw streams, and performance slot count `0` are required.
...
`30`はharness資格確認だけを行い、performance slot、KBM、旧stage再利用は禁止する。`10`はgame code／値／
profiling seamを変更しない。資格確認後は必ず`00`へ戻し、performance測定は別票まで行わない。
